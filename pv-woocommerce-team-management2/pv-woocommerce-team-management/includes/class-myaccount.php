...
class WC_Team_Payroll_MyAccount {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_account_my-salary-details_endpoint', array( $this, 'render_salary_details_tab' ) );
		add_action( 'woocommerce_account_my-earnings_endpoint', array( $this, 'render_earnings_tab' ) );
		add_action( 'woocommerce_account_my-orders-commission_endpoint', array( $this, 'render_orders_tab' ) );
		add_action( 'woocommerce_account_my-reports_endpoint', array( $this, 'render_reports_tab' ) );
		add_action( 'wp_ajax_wc_tp_get_orders_data', array( $this, 'ajax_get_orders_data' ) );
		add_action( 'wp_ajax_wc_tp_get_order_details', array( $this, 'ajax_get_order_details' ) );
	}
	...
	public function add_menu_items( $items ) {
		...
		if ( ! $user_id || ! $this->user_has_agent_role( $user_id ) ) {
			return $items;
		}
	...
	public function render_salary_details_tab() {
		$user_id = get_current_user_id();
		if ( ! $user_id ) {
			return;
		}
		...
		if ( ! $this->user_has_agent_role( $user_id ) ) {
	...
	public function render_earnings_tab() {
		$user_id = get_current_user_id();
		if ( ! $user_id ) {
			return;
		}
	...
	public function render_reports_tab() {
		...
		$engine = new WC_Team_Payroll_Core_Engine();
		$current_month = $engine->get_user_earnings( $user_id );
	 ...
	 * Check if user has one of the selected agent roles
	 */
	private function user_has_agent_role( $user_id ) {
	 ...
	 */
	public function ajax_get_orders_data() {
		...
		foreach ( $orders as $order ) {
			...
			// Get "Made By" name
			$made_by = $this->get_made_by_name( $user_id, $agent_id, $processor_id );
	 ...
	 * Get "Made By" name
	 */
	private function get_made_by_name( $current_user_id, $agent_id, $processor_id ) {
	...
	public function ajax_get_order_details() {
		...
		$engine = new WC_Team_Payroll_Core_Engine();
		$change_log = $engine->get_order_change_log( $order_id );
