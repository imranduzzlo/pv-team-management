...
class WC_Team_Payroll_Core_Engine {
	...
	public function __construct() {
		...
		add_action( 'before_delete_post', array( $this, 'on_order_trashed' ) );
	}
	 ...
	 * Apply extra earnings rules
	 */
	private function apply_extra_earnings( $order, $commission_data ) {
		$settings = get_option( 'wc_team_payroll_settings', array() );
	 ...
	 */
	private function check_rule_condition( $order, $rule, $commission_data ) {
	 ...
	 * Log order changes
	 */
	private function log_order_change( $order_id, $old_data, $new_data ) {
		$order = wc_get_order( $order_id );
		if ( ! $order ) {
			return;
		}
	 ...
	 */
	private function calculate_refund_commission( $order, $agent_id, $processor_id ) {
		$settings = get_option( 'wc_team_payroll_settings', array() );
	 ...
	 */
	public function on_order_trashed( $post_id ) {
	 ...
	 */
	private function log_order_cancellation( $order_id, $old_data, $reason ) {
		$order = wc_get_order( $order_id );
		if ( ! $order ) {
			return;
		}
	 ...
	 */
	public function get_order_change_log( $order_id ) {
		$order = wc_get_order( $order_id );
		if ( ! $order ) {
			return array();
		}
