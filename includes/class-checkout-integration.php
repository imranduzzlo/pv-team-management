...
class WC_Team_Payroll_Checkout_Integration {
	...
	private static $agent_field_name = 'order_other_agent_or_not';
	private static $processor_field_name = '_processor_user_id';
	private static $agent_user_roles = array( 'shop_employee', 'shop_manager', 'administrator' );

	public static function init() {
		self::load_settings();
		add_action( 'wp_footer', array( __CLASS__, 'auto_populate_agent_dropdown' ), 99 );
		add_action( 'woocommerce_checkout_create_order', array( __CLASS__, 'store_agent_processor_meta' ), 20, 2 );
	}
	...
	private static function load_settings() {
		...
		if ( isset( $checkout_fields['agent_field_name'] ) ) {
			self::$agent_field_name = sanitize_text_field( $checkout_fields['agent_field_name'] );
		}

		if ( isset( $checkout_fields['processor_field_name'] ) ) {
			self::$processor_field_name = sanitize_text_field( $checkout_fields['processor_field_name'] );
		}

		if ( isset( $checkout_fields['agent_user_roles'] ) && is_array( $checkout_fields['agent_user_roles'] ) ) {
			self::$agent_user_roles = array_map( 'sanitize_text_field', $checkout_fields['agent_user_roles'] );
		}
	}

	public static function auto_populate_agent_dropdown() {
		if ( ! is_checkout() ) {
			return;
		}
		...
		if ( $current_user_id ) {
			...
			if ( $user ) {
				foreach ( self::$agent_user_roles as $role ) {
		...
		$users = get_users( array(
			'role__in' => self::$agent_user_roles,
		...
		<script>
			(function($) {
				...
				window.__AGENT_FIELD_NAME__ = '<?php echo esc_js( self::$agent_field_name ); ?>';
	...
	public static function store_agent_processor_meta( $order, $data ) {
		$current_user_id = is_user_logged_in() ? get_current_user_id() : null;
		$agent_id = ! empty( $_POST[ self::$agent_field_name ] ) ? intval( $_POST[ self::$agent_field_name ] ) : null;
