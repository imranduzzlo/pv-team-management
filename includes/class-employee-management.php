...
class WC_Team_Payroll_Employee_Management {
	...
	public static function init() {
		add_action( 'admin_menu', array( __CLASS__, 'add_employee_menu' ) );
		add_action( 'admin_init', array( __CLASS__, 'register_employee_settings' ) );
		add_action( 'wp_ajax_wc_tp_update_employee_salary', array( __CLASS__, 'ajax_update_employee_salary' ) );
		add_action( 'wp_ajax_wc_tp_add_payment', array( __CLASS__, 'ajax_add_payment' ) );
		add_action( 'wp_ajax_wc_tp_delete_payment', array( __CLASS__, 'ajax_delete_payment' ) );
		add_action( 'wp_ajax_wc_tp_add_order_bonus', array( __CLASS__, 'ajax_add_order_bonus' ) );
		add_action( 'wp_ajax_wc_tp_get_payment_data', array( __CLASS__, 'ajax_get_payment_data' ) );
	}

	public static function add_employee_menu() {
		add_submenu_page(
			'wc-team-payroll',
			...
			array( __CLASS__, 'render_employees_page' )
		);
	}

	public static function register_employee_settings() {
	...
	public static function ajax_update_employee_salary() {
		...
		// Add to history
		self::add_salary_history( $user_id, $old_salary_type, $old_amount, $old_frequency, $salary_type, $salary_amount, $salary_frequency );
	...
	private static function add_salary_history( $user_id, $old_type, $old_amount, $old_frequency, $new_type, $new_amount, $new_frequency ) {
	...
	public static function ajax_get_payment_data() {
		check_ajax_referer( 'wc_team_payroll_nonce', 'nonce' );
	...
	public static function ajax_add_order_bonus() {
		check_ajax_referer( 'wc_team_payroll_nonce', 'nonce' );
